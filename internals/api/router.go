...
func NewRouter(billingHandler *billing.Handler, aggregationHandler *aggregation.Handler, analyticsHandler *analytics.Handler, accountHandler *account.Handler, recommendation *recommendation.Handler) http.Handler {
	r := chi.NewRouter()
	handler := NewHandlers(billingHandler, aggregationHandler, analyticsHandler, accountHandler, recommendation) // ← pass billing service here
